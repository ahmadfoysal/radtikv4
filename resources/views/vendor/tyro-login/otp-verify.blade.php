        ...
        @if (in_array($layout, ['split-left', 'split-right']))
            <div class="background-panel" style="background-image: url('{{ $backgroundImage }}');">
                <div class="background-panel-content">
                    ...
                    <p>{{ $otpConfig['background_description'] ?? 'Enter the verification code we sent to your email to complete the login process.' }}
                    </p>
                </div>
            </div>
        @endif
            ...
            <div class="form-card">
                ...
                <form method="POST" action="{{ route('tyro-login.otp.submit') }}">
                    ...
                    <div class="form-group">
                        <label for="otp"
                            class="form-label text-center">{{ $otpConfig['label'] ?? 'Verification Code' }}</label>
                        <div class="otp-input-container">
                            @for ($i = 0; $i < $otpLength; $i++)
                                <input type="text" class="otp-digit @error('otp') is-invalid @enderror" maxlength="1"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            digits.forEach((digit, index) => {
                ...
                digit.addEventListener('paste', function(e) {
                    ...
                    const numbers = pastedData.replace(/[^0-9]/g, '').split('').slice(0, digits
                        .length);
