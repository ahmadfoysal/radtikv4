        ...
        @if (in_array($layout, ['split-left', 'split-right']))
            <div class="background-panel" style="background-image: url('{{ $backgroundImage }}');">
                <div class="background-panel-content">
                    ...
                    <p>{{ $pageContent['background_description'] ?? 'Sign in to access your account and continue where you left off. We\'re glad to see you again.' }}
                    </p>
                </div>
            </div>
        @endif
        ...
        <div class="form-panel">
            <div class="form-card">
                ...
                @if (session('error') || request('error') === 'account_suspended')
                    <div
                        style="background-color: #fee2e2; border: 1px solid #fecaca; color: #991b1b; padding: 12px; border-radius: 6px; margin-bottom: 16px;">
                        @if (request('error') === 'account_suspended')
                    ...
                    @if (($loginField ?? 'email') === 'both')
                        <div class="form-group">
                            ...
                            <input type="text" id="login" name="login"
                                class="form-input @error('login') is-invalid @enderror" value="{{ old('login') }}" required
                                autocomplete="username" autofocus placeholder="Email or username">
                            @error('login')
                    ...
                    @else
                        <div class="form-group">
                            ...
                            <input type="email" id="email" name="email"
                                ...
                                required autocomplete="email" autofocus placeholder="email@example.com">
                            @error('email')
                                <span class="error-message">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    @if ($captchaEnabled ?? false)
                        <div class="form-group captcha-group">
                            <label for="captcha_answer"
                                class="form-label">{{ $captchaConfig['label'] ?? 'Security Check' }}</label>
                            <div class="captcha-container">
                                <span class="captcha-question">{{ $captchaQuestion }}</span>
                                <input type="number" id="captcha_answer" name="captcha_answer"
                                    class="form-input captcha-input @error('captcha_answer') is-invalid @enderror" required
                                    autocomplete="off"
