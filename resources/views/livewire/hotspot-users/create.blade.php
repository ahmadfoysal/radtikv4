<x-mary-card title="Create Single Hotspot User" separator class="max-w-4xl mx-auto bg-base-100">
    ...
    <div class="relative">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="col-span-1 sm:col-span-2">
                <x-mary-select label="Hotspot Profile (Optional)" icon="o-wifi" wire:model="profile" :options="$available_profiles"
                    option-label="name" option-value="id" placeholder="Select a profile (optional)" :disabled="!$router_id || empty($available_profiles)" />
                @if ($router_id && empty($available_profiles))
                    <p class="text-xs text-warning mt-1">No profiles found. Please create a profile in the Profile
                        Management section first.</p>
                @endif
            </div>
            ...
            {{-- Password --}}
            <x-mary-input label="Password" type="password" wire:model="password" placeholder="Enter password"
                icon="o-key" />
