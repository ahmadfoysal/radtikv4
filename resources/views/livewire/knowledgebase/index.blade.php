    ...
    <x-mary-card class="mb-6 bg-base-100 border border-base-300 shadow-sm">
        <div class="px-4 py-4 flex flex-col gap-3">
            <div class="flex items-center gap-2 mb-2">
                <x-mary-icon name="o-light-bulb" class="w-6 h-6 text-primary" />
                <span class="font-semibold text-lg">Knowledge Base</span>
            </div>

            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                {{-- Search Box --}}
                <x-mary-input placeholder="Search articles..." icon="o-magnifying-glass" class="w-full sm:flex-1"
    ...
    <div class="px-2 sm:px-4">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @forelse ($articles as $article)
                <x-mary-card class="bg-base-100 shadow-sm hover:shadow-md transition duration-300 border border-base-300">
                    <div class="space-y-3">
                        {{-- Category Badge --}}
                        <div>
                        ...
                        <div class="flex items-center justify-between pt-2">
                            ...
                            <x-mary-button icon="o-arrow-right" label="Read More" class="btn-xs btn-ghost"
                                href="{{ route('knowledgebase.show', $article->slug) }}" wire:navigate />
                        </div>
                    </div>
                </x-mary-card>
            @empty
