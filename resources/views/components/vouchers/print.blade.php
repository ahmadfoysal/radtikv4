...
<body class="bg-gray-50 p-6 min-h-screen">
    ...
    <!-- Action bar -->
    <div class="no-print fixed top-0 left-0 right-0 bg-white shadow-lg p-4 flex justify-between items-center z-50 border-b-2 border-gray-200">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">WiFi Vouchers - {{ $voucherCount }} Cards</h1>
            <p class="text-sm text-gray-600 mt-1">{{ $router->name }} • {{ $router->login_address ?? 'No login address' }} • 48 cards per page (A4 Landscape)</p>
        </div>
        <div class="flex gap-3">
            <button onclick="window.history.back()" class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-100 transition">
                ← Back
            </button>
            <button onclick="window.print()" class="px-6 py-2 bg-black text-white rounded-lg hover:bg-gray-800 transition font-semibold">
                🖨️ Print / Save PDF
            </button>
        </div>
    </div>
