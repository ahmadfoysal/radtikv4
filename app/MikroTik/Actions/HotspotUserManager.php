...
class HotspotUserManager
{
    ...
    /**
     * Reset the router-side session state for a voucher.
     *
     * RADIUS owns the MAC binding, so this only removes all matching
     * active sessions and cookies from MikroTik.
     */
    public function resetVoucher(Router $router, string $username): array
    {
        ...
        // A voucher authenticated through RADIUS may not have a local
        // /ip/hotspot/user entry, so do not require one before cleanup.
        try {
            ...
            $actions[] = $removedCookies > 0
                ? "Removed {$removedCookies} session cookie(s)"
                : 'No session cookies found';
        } catch (\Throwable $e) {
            $errors[] = 'Error removing cookie: ' . $e->getMessage();
        }

        // Remove every active session for the username, not just the first.
        try {
            $activeSessions = $this->getActiveSessions($router);
            $removed = 0;
            foreach ($activeSessions as $session) {
                if (($session['user'] ?? null) !== $username || empty($session['.id'])) {
                    continue;
                }
            ...
            $actions[] = $removed > 0
                ? "Removed {$removed} active session(s)"
